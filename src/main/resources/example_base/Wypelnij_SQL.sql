-- CREATE VIEW KONTA_START AS SELECT LOGIN,PASSWORD,ACCOUNT_FUNCTION FROM ACCOUNT WHERE ACTIVE=1;

DELETE FROM WYP.ACCOUNT;
DELETE FROM WYP.ACCOUNT_FUNCTION;
DELETE FROM WYP.EQUIPMENT;
DELETE FROM WYP.EQUIPMENT_STATUS;
DELETE FROM WYP.LICENSE_TYPE;
DELETE FROM WYP.RESERV;
DELETE FROM WYP.RESERV_STATUS;
DELETE FROM WYP.USE_PLACE;

INSERT INTO WYP.ACCOUNT (ID, ACCOUNT_FUNCTION, ACTIVE, EMAIL, LOGIN, NAME, PASSWORD, PHONE, SURNAME) 
VALUES (1, 'A', 'y', 'test@test.pl', 'admin', 'Przemek', 'admin', '111111111', 'Przyslo');

INSERT INTO WYP.ACCOUNT (ID, ACCOUNT_FUNCTION, ACTIVE, EMAIL, LOGIN, NAME, PASSWORD, PHONE, SURNAME) 
VALUES (2, 'K', 'y', 'kierownik@kierownik.pl', 'kierownik', 'Jan', 'kierownik', '222222222', 'Tomaszak');

INSERT INTO WYP.ACCOUNT (ID, ACCOUNT_FUNCTION, ACTIVE, EMAIL, LOGIN, NAME, PASSWORD, PHONE, SURNAME) 
VALUES (3, 'P', 'y', 'pracownik@pracownik.pl', 'pracownik', 'Zenon', 'pracownik', '333333333', 'Pasztet');

INSERT INTO WYP.ACCOUNT_FUNCTION (ID, FUNCTION_NAME, ACCOUNT_ID) 
VALUES (1, 'Administrator', 1);

INSERT INTO WYP.ACCOUNT_FUNCTION (ID, FUNCTION_NAME, ACCOUNT_ID) 
VALUES (2, 'Kierownik', 2);

INSERT INTO WYP.ACCOUNT_FUNCTION (ID, FUNCTION_NAME, ACCOUNT_ID) 
VALUES (3, 'Pracownik', 3);

INSERT INTO WYP.EQUIPMENT_STATUS (ID, EQUIPMENT_STATUS_NAME) 
VALUES (1, 'Sprawny');

INSERT INTO WYP.EQUIPMENT_STATUS (ID, EQUIPMENT_STATUS_NAME) 
VALUES (2, 'W naprawie');

INSERT INTO WYP.EQUIPMENT_STATUS (ID, EQUIPMENT_STATUS_NAME) 
VALUES (3, 'Niekompletny');

INSERT INTO WYP.EQUIPMENT_STATUS (ID, EQUIPMENT_STATUS_NAME) 
VALUES (4, 'Uszkodzony');

INSERT INTO WYP.USE_PLACE (ID, BUILDING, FLOOR, PLACE) 
VALUES (1, 'Budynek A', 1, 'sala208');

INSERT INTO WYP.USE_PLACE (ID, BUILDING, FLOOR, PLACE) 
VALUES (2, 'Budynek A', 1, 'sala201');

INSERT INTO WYP.USE_PLACE (ID, BUILDING, FLOOR, PLACE) 
VALUES (3, 'Budynek B', 0, 'sala516');

INSERT INTO WYP.LICENSE_TYPE (ID, LICENSE_END, LICENSE_NAME, LICENSE_SERIAL, LICENSE_START) 
VALUES (1, '2017-11-01', 'OEM', 'asdfg-qwert', '2015-11-01');

INSERT INTO WYP.LICENSE_TYPE (ID, LICENSE_END, LICENSE_NAME, LICENSE_SERIAL, LICENSE_START) 
VALUES (2, '2016-01-01', 'BOX', 'ghjkl-poi87', '2010-01-01');

INSERT INTO WYP.LICENSE_TYPE (ID, LICENSE_END, LICENSE_NAME, LICENSE_SERIAL, LICENSE_START) 
VALUES (3, '2017-06-15', 'Educational', 'zxcvb-12345', '2019-01-01');

INSERT INTO WYP.RESERV_STATUS (ID, RESERVATION_STATUS_NAME) 
VALUES (1, 'Zarezerwowany');

INSERT INTO WYP.RESERV_STATUS (ID, RESERVATION_STATUS_NAME) 
VALUES (2, 'Wypozyczony');

INSERT INTO WYP.RESERV_STATUS (ID, RESERVATION_STATUS_NAME) 
VALUES (3, 'Dostepny');

INSERT INTO WYP.RESERV (ID, DESCRIPTION, RESERVATION_END, RESERVATION_MADE_DATE, RESERVATION_START, ACCOUNT_ID, RESERV_STATUS_ID) 
VALUES (1, 'Do celow szkoleniowych', '2017-09-01', '2017-06-01', '2017-07-01', 1, 1);

INSERT INTO WYP.RESERV (ID, DESCRIPTION, RESERVATION_END, RESERVATION_MADE_DATE, RESERVATION_START, ACCOUNT_ID, RESERV_STATUS_ID) 
VALUES (2, 'Do celów prywatnych', '2017-12-01', '2017-06-15', '2017-07-15', 2, 2);

INSERT INTO WYP.RESERV (ID, DESCRIPTION, RESERVATION_END, RESERVATION_MADE_DATE, RESERVATION_START, ACCOUNT_ID, RESERV_STATUS_ID) 
VALUES (3, 'Wyjazd sluzbowy', '2017-06-29', '2017-06-15', '2017-06-20', 3, 1);

INSERT INTO WYP.EQUIPMENT (ID, NAME, RENT_PERMISSION, TYPE, EQUIPMENT_STATUS_ID, LICENSE_TYPE_ID, RESERV_ID, USE_PLACE_ID) 
VALUES (1, 'Komputer', 't', 'stacjonarny',1, 1, 1, 1);

INSERT INTO WYP.EQUIPMENT (ID, NAME, RENT_PERMISSION, TYPE, EQUIPMENT_STATUS_ID, LICENSE_TYPE_ID, RESERV_ID, USE_PLACE_ID) 
VALUES (2, 'Laptop1', 't', 'przenosny',1, 2, 2, 2);

INSERT INTO WYP.EQUIPMENT (ID, NAME, RENT_PERMISSION, TYPE, EQUIPMENT_STATUS_ID, LICENSE_TYPE_ID, RESERV_ID, USE_PLACE_ID) 
VALUES (3, 'Rzutnik', 'n', 'stacjonarny',1, 2, 2, 3);


